...
- **Rule 4 (NEW)**: Khi viết ví dụ đầu vào/đầu ra trong `inputbox`/`outputbox`, phải dùng `\begin{samplecode}...\end{samplecode}` thay vì `\begin{lstlisting}...\end{lstlisting}`. `samplecode` đã được định nghĩa trong template with `aboveskip=0pt, belowskip=0pt, frame=none` để tránh double-padding.
- **Rule 5 (NEW)**: BẮT BUỘC ghi lại comment chứa `order_index` ở ngay dòng đầu tiên của file `.tex` đầu ra theo định dạng chính xác: `% order_index: <N>` (ví dụ: `% order_index: 3`). Đây là điều kiện tiên quyết để script gộp LaTeX sắp xếp đúng thứ tự các bài toán.
